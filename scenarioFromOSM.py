...
def _call_generate_parking_areas_from_osm(filename):
    ...
    parking_options = ['--osm', filename, '--net',
                       DEFAULT_NET_XML, '--out', DEFAULT_PARKING_AREAS]
...
def main(cmd_args):
    ...
    if args.from_step <= 0 and args.to_step >= 0:
        msg = 'Copying default configuration files to destination.'
        print(msg)
        report(msg)
    ...
    if args.from_step <= 1 and args.to_step >= 1:
        msg = 'Generate the net.xml with all the additional components ' \
              '(public transports, parkings, ..)'
        print(msg)
        report(msg)

        _call_netconvert(args.osm_file, args.left_hand_traffic)
        msg = 'Generate polygons using polyconvert.'
        print(msg)
        report(msg)
        _call_polyconvert(args.osm_file)

    if args.from_step <= 2 and args.to_step >= 2:
        msg = 'Generate flows for public transportation using ptlines2flows.'
        print(msg)
        report(msg)
        _call_pt_lines_to_flows()

    if args.from_step <= 3 and args.to_step >= 3:
        msg = 'Generate parking area location and possibly merge it with the one provided ' \
              'by netconvert.'
        print(msg)
        report(msg)
    ...
    if args.from_step <= 4 and args.to_step >= 4:
        msg = 'Generate parking area rerouters using tools/generateParkingAreaRerouters.py'
        print(msg)
        report(msg)
        _call_generate_parking_area_rerouters(args.processes)

    if args.from_step <= 5 and args.to_step >= 5:
        msg = 'Extract taxi stands from OpenStreetMap.'
        print(msg)
        report(msg)
        _call_generate_taxi_stands_from_osm(args.osm_file)

    if args.from_step <= 6 and args.to_step >= 6:
        msg = 'Generate taxi stands rerouters using tools/generateParkingAreaRerouters.py'
        print(msg)
        report(msg)
        _call_generate_parking_area_rerouters_for_stands(args.processes)

    if args.from_step <= 7 and args.to_step >= 7:
        msg = 'Generate TAZ from administrative boundaries, TAZ weights using buildings and ' \
    ...
    if args.from_step <= 8 and args.to_step >= 8:
        msg = 'Generate the default OD-Matrix in Amitran format. '
        print(msg)
        report(msg)
    ...
    if args.from_step <= 9 and args.to_step >= 9:
        msg = 'Generate the default values for the activity based mobility generator. '
        print(msg)
        report(msg)
    ...
    if args.from_step <= 10 and args.to_step >= 10:
        msg = 'Mobility generation using SUMOActivityGen.'
        print(msg)
        report(msg)
        _call_activitygen()
