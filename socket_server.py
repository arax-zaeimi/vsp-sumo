...
class Builder(object):
    ...
    def __init__(self, data, local):
        ...
        # self.tmp = os.path.abspath(const.FILES_PATH)
    ...
    def build(self):
        ...
        if self.data["vehicles"] or ptOptions:
            ...
            self.edges = sumolib.net.readNet(os.path.join(
                self.tmp, self.files["net"])).getEdges()
            ...
            for vehicle, options in self.data["vehicles"].items():
                ...
                try:
                    options = self.parseTripOpts(
                        vehicle, options, self.data["publicTransport"])
                except ZeroDivisionError:
                    continue

                if vehicle == "pedestrian" and self.data["publicTransport"]:
                    options += ["--additional-files",
...
parser.add_argument("--remote", action="store_true",
                    ...
                    "will be generated.")
parser.add_argument("--osm-file", default="osm_bbox.osm.xml",
                    dest="osmFile", help="use input file from path.")
